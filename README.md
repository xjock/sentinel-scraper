...
首次运行时，如果 `~/.sentinel-scraper/settings.json` 不存在，程序会自动启动本地 HTTP 服务并打开浏览器，引导你选择数据源和填写认证信息。配置完成后程序自动继续。
...
配置文件保存在 `~/.sentinel-scraper/settings.json`，权限 `0600`（仅所有者可读写）。**密码以明文存储**，请妥善保护用户目录权限。
...
# 或用 Makefile
make build           # 等价于 go build -o sentinel-scraper .
make run             # 构建并运行
make fmt             # go fmt ./...
make vet             # go vet ./...
make clean           # 清理产物
